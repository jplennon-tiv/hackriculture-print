...
const root=path.resolve(import.meta.dirname,'..'),out=path.join(root,'output/pdf/how-to-centred-v2');
...
const placements=[
 [{x:0,y:63,w:43,side:'left'},{x:148,y:72,w:42,side:'right'},{x:148,y:124,w:42,side:'right'},{x:0,y:130,w:43,side:'left'},{x:57,y:197,w:80,side:'bottom'}],
 [{x:148,y:36,w:42,side:'right'},{x:0,y:94,w:43,side:'left'},{x:148,y:94,w:42,side:'right'},{x:148,y:151,w:42,side:'right'}]
];
function marks(items,index){return items.map(({n,oval},i)=>{
 const a=placements[index][i],factor=86/118;
 const [ox,oy,orx,ory]=oval;
 const cx=52+ox*factor,cy=60+(oy-20)*factor,rx=orx*factor,ry=ory*factor;
 let sx=cx+rx,sy=cy,ex=a.x-2,ey=a.y+5;
 if(a.side==='left'){sx=cx-rx;ex=a.x+a.w+2;}
 if(a.side==='bottom'){sx=cx;sy=cy+ry;ex=cx;ey=a.y-3;}
 const mid=(sx+ex)/2;
 const connector=a.side==='bottom'?'M '+sx+' '+sy+' L '+ex+' '+ey:'M '+sx+' '+sy+' C '+mid+' '+sy+','+mid+' '+ey+','+ex+' '+ey;
 return `<g fill="none" stroke="#b55727" stroke-width=".55" stroke-linecap="round"><path d="M ${cx-rx} ${cy} C ${cx-rx-1} ${cy-ry-1},${cx+rx-2} ${cy-ry-1},${cx+rx} ${cy} C ${cx+rx+1} ${cy+ry+1},${cx-rx+1} ${cy+ry+1},${cx-rx} ${cy} Z"/><path d="${connector}" stroke-width=".4" marker-end="url(#arrow${index})"/></g><circle cx="${cx-rx+2}" cy="${cy-ry-1}" r="2.4" fill="#b55727"/><text x="${cx-rx+2}" y="${cy-ry}" text-anchor="middle" fill="white" font-family="Inter,sans-serif" font-size="2.8" font-weight="800">${n}</text>`;
 }).join('');}
function page(index,items){return `<section class="page"><header><div class="eyebrow">HOW TO USE YOUR CHEAT SHEETS / ${index+1} OF 2</div><h1>${index?'Turn over. Get growing.':'Start here. Plan your crop.'}</h1><p>${index?'Planting, care and picking - explained at a glance.':'The front brings your crop’s essentials together.'}</p></header><main><div class="sheetlabel">EXAMPLE SHEET · ${index?'BACK / PAGE 2':'FRONT / PAGE 1'}</div><svg class="drawing" viewBox="0 0 190 225" xmlns="http://www.w3.org/2000/svg"><defs><marker id="arrow${index}" viewBox="0 0 6 6" refX="5" refY="3" markerWidth="5" markerHeight="5" orient="auto-start-reverse"><path d="M 1 1 L 5 3 L 1 5" fill="none" stroke="#b55727" stroke-width="1"/></marker></defs><svg x="52" y="60" width="86" height="122.44" viewBox="58 76 766 1090"><image href="${sheets[index]}" width="884" height="1250"/></svg>${marks(items,index)}</svg>${items.map((a,i)=>{const p=placements[index][i];return `<article style="left:${p.x}mm;top:${p.y}mm;width:${p.w}mm"><h2><b>${a.n}</b>${a.h}</h2>${a.graphic}<p>${a.p}</p></article>`}).join('')}<div class="under-sheet">${index?'Use the practical steps while you work.':'Read your own crop’s figures; the sheet here is an example.'}</div></main><footer><span>VEGETABLE CHEAT SHEETS</span><span>HOW TO USE · ${index+1} / 2 · DESIGN PROOF</span></footer></section>`;}
...
.drawing{height:225mm}main{height:225mm;margin-top:1mm;margin-left:10mm}.sheetlabel{left:52mm;top:52mm;font-size:6.3pt;letter-spacing:.5pt}.under-sheet{left:0;top:8mm;width:190mm;text-align:center;font-size:9pt}.iconrow{gap:2mm}.iconrow img{width:9mm;height:9mm}h2{font-size:9pt;margin-bottom:2.5mm}article p{font-size:8pt;line-height:1.45}.months{word-spacing:1.4mm}.sample{font-size:6pt}.key{font-size:6pt}
