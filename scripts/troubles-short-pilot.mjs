...
import copy from '../docs/troubles-design/pilot-copy.mjs';
const root=path.resolve(import.meta.dirname,'..');
const out=path.join(root,'output/pdf/troubles-pilot-v2');
...
const keys=['carrot_fly','green_top','parsnip_canker','old_seed_poor_germination'];
const esc=s=>String(s??'').replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;').replaceAll('"','&quot;');
const cards=[];
for(const key of keys){
 const c={...group.conditions[key],...copy[key]};let art='';
...
const page=(variant,title,body)=>`<div class="page ${variant}"><header><div class="eyebrow">VEGETABLE CHEAT SHEETS / TROUBLES</div><h1>Carrot &amp; parsnip troubles</h1><div class="deck">Recognise the signs. Find the next step.</div></header><div class="concept"><b>${title}</b><span>Four selected entries · draft summaries</span></div><main>${body}</main><footer><span>EDITORIAL PILOT · SHORT PRINT COPY FOR REVIEW</span><span>1 / 1</span></footer></div>`;
...
.columns .copy section{padding:2mm 2.5mm;border:0;border-left:2px solid #a2bac2;border-radius:1mm;background:#eef3f5}.columns .copy section.act{background:#fbefdf;border-color:#cc985d}.columns .copy section.prevent{background:#edf3e6;border-color:#92a77a}.columns .recognise h3{color:#405e6a}.columns .act h3{color:#845324}.columns .prevent h3{color:#4a684b}.columns figure img{height:25mm}.columns .copy section+section{margin-top:1.5mm}.columns p{font-size:8.8pt;line-height:1.38}.columns .card-body{padding:2.5mm}.columns h2{font-size:10.5pt}.column{gap:3mm}
</style><body>${page('columns','Recognise / Act / Prevent',`<div class="column">${cards[0]}${cards[1]}</div><div class="column">${cards[2]}${cards[3]}</div>`)}</body></html>`;
...
try{
 ...
 // Ensure the PDF matches the stored editorial draft exactly.
 for(const variant of ['columns'])for(const key of keys)for(const [label,field] of [['Recognise','description'],['Act','treatment'],['Prevent','prevention']]){
  const value=copy[key][field];if(!value)continue;
 ...
 await p.pdf({path:path.join(out,'troubles-short-copy-pilot-A4.pdf'),format:'A4',preferCSSPageSize:true,printBackground:true});
