...
const out=new URL('../output/pdf/planting-rollout05/',import.meta.url);
await fs.mkdir(out,{recursive:true});
const crops=['beet_leaf','bean_runner','pea','marrow_courgette','squash_pumpkin','cucumber_outdoor','cucumber_greenhouse'];
const browser=await chromium.launch();const results=[];
try {
 const page=await browser.newPage({viewport:{width:688,height:979}});
 for(const crop of crops){
  const url=`http://127.0.0.1:5173/print/vegetable/${crop}?units=metric`;
  ...
  const review=normal.layout==='legacy'||!!normal.error||!normal.layout.endsWith(':images');
  if(review){await page.goto(url+'&plantingReview=1',{waitUntil:'networkidle'});await page.waitForFunction(()=>document.body.dataset.printReady==='true'||!!document.body.dataset.printError);}
