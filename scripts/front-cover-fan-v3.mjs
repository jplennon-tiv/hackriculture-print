...
const out=path.join(root,'output/pdf/front-cover-fan-v3');
await fs.mkdir(out,{recursive:true});
const original=await fs.readFile(path.join(root,'output/pdf/front-matter-concepts/front-matter-concepts.html'),'utf8');
const image='data:image/png;base64,'+(await fs.readFile(path.join(root,'docs/front-matter/assets/hands-sheet-fan-v3.png'))).toString('base64');
...
try {
 ...
 await page.evaluate(image=>{
  ...
  document.querySelector('footer span:last-child').textContent='FRONT COVER / FAN STUDY 03';
 ...
 await page.pdf({path:path.join(out,'vegetable-cheat-sheets-cover-fan-v3.pdf'),format:'A4',printBackground:true,preferCSSPageSize:true});
 console.log(out);
