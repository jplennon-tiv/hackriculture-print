...
const out=path.join(root,'output/pdf/front-cover-fan-v2');
await fs.mkdir(out,{recursive:true});
const original=await fs.readFile(path.join(root,'output/pdf/front-matter-concepts/front-matter-concepts.html'),'utf8');
const image='data:image/png;base64,'+(await fs.readFile(path.join(root,'docs/front-matter/assets/hands-sheet-fan-v2.png'))).toString('base64');
...
try {
 ...
 await page.evaluate(image=>{
  ...
  document.querySelector('footer span:last-child').textContent='FRONT COVER / FAN STUDY 02';
 ...
 await page.pdf({path:path.join(out,'vegetable-cheat-sheets-cover-fan-v2.pdf'),format:'A4',printBackground:true,preferCSSPageSize:true});
 console.log(out);
